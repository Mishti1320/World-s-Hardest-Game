...
var obs = {
  ...
  borderCheck: function(x, y) {
    for (let i = 0; i < 8; i ++) {
      if (lvl1 == true) {
				if (obs.xcors[i] <= x + obsCheck || obs.xcors[i] >= width - x - obsCheck || obs.ycors[i] <= y + obsCheck || obs.ycors[i] >= height - y - obsCheck) {
      	  obs.spdY[i] = -obs.spdY[i]
      	}
      }
    	if (lvl2 == true) {
      	if (obs.xcors2[i] <= x + obsCheck || obs.xcors2[i] >= width - x - obsCheck || obs.ycors2[i] <= y + obsCheck || obs.ycors2[i] >= height - y - obsCheck)
      		obs.spdX[i] = -obs.spdX[i]
      	}
      if (lvl3 == true) {
      	if (obs.xcors3[i] <= x + obsCheck || obs.xcors3[i] >= width - x - obsCheck || obs.ycors3[i] <= y + obsCheck || obs.ycors3[i] >= height - y - obsCheck)
      		if (i % 2 == 0) {
            obs.spdX[i] = -obs.spdX[i]
          }
          else {
